...
This evaluation uses \textit{validation set} pre-train the model greedy layer-wise, \textit{train set} to train the whole neural network and \textit{test set} to evaluate the with different evaluation parameters.\\ \par
  ...
  As we can see that with \textit{Adam Optimizer}, only the \textit{sigmoid activation} was able to classify correctly different attack types,\text{ ReLU, eLU, and Softpuls} were unable to classify single \textit{Probing, U2R and I2R} attacks.
%%%%%%%%%%%%%%%% sigmoid adam tf %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%	
   \subsection{Sigmoid Activation}
 	 \begin{table}[ht]
		\centering
